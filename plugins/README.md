...
| tool | tool-im | IM 远程工具(G-E5-3):im_send(向**已授权**用户/群投递文本)/im_status(连接·模型·会话·可投目标);**默认不注册**(data.enabled),目标经 ctx.imControl 校验(未授权拒绝,不隐式回落);建议配 policy-guard data.approval_tools: [im_send] | ctx.tools + ctx.imControl(运行期现取) |
