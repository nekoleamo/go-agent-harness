...
// IdleMarker 外部插件「自述空闲」的标记前缀(插件与宿主共用的单一真源)。
// 语义:插件本轮**主动不参与**加载(如工具类插件还没配置任何后端)—— 这不是故障,
// 所以宿主记 INFO 跳过、不往启动日志里丢 ERROR。插件应把它写在 stderr 并以 0 退出:
// 未使用某功能不该在用户面前呈现为「插件加载失败」。
