...
| `im_send` / `im_send_file` / `im_send_page` / `im_status` | **Off by default**: send a message, send a **workspace file** (registered then delivered; size cap `data.media_max_mb`, default 20MB), send a **PDF page rendered as an image** (requires `data.external_raster`) to an **authorized** IM user/group, and query the controllable state. Enabling requires `enabled: true` + `data.enabled: true` on `tool-im` and adding `im_send`/`im_send_file`/`im_send_page` to `policy-guard`'s `data.approval_tools` (per-call confirmation in smart mode) |
