...
| **Desktop shell (P1)** | `desktop/` Tauri v2 shell: sidecar gah + window wired straight to the local service; tray (about / check updates / autostart) / notify / single-instance / system folder picker; **zero-cost release** (updater with self-held ed25519 signing + CI matrix + unsigned-run first-launch guide) |
