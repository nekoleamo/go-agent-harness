...
| **Desktop shell (P1)** | `desktop/` Tauri v2 shell: sidecar gah + window wired straight to the local service; tray (about / check updates / autostart) / notify / single-instance / system folder picker; **diagnostics**: `<user data>/gah-shell.log` collects shell events, sidecar stderr (go-plugin `[DEBUG]` filtered) and page-side errors (reported by the frontend via `shell_log`); **zero-cost release** (updater with self-held ed25519 signing + CI matrix + unsigned-run first-launch guide) |
