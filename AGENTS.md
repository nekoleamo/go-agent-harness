...
- 思考等级:Tab=前进/Shift+Tab=后退循环(off→low→medium→high),/thinking 命令(枚举选择器),状态栏"思维: <等级>"(off 隐藏);host-llm 会话级注入(请求未显式时),openai→reasoning_effort / anthropic→thinking budget_tokens(1024/4096/16384),off 均不发送(安全默认,兼容不支持端点)。
