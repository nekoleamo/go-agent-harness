...
- P4 发行平台匹配:gen-extplugins.sh 按发行矩阵五目标构建外部插件→embed 按平台拆包(build-tag 同名变量,主包只嵌本平台产物,体积门不变);CI 交叉编译矩阵(五目标+每目标体积门+TestExtPluginsMatchPlatform 魔数/架构校验);tests 经 embed.OpenExtPlugin 读本平台产物。修改 embed 布局后需重跑 scripts/gen-extplugins.sh(旧平铺 *.gz 会被清,embed 目录缺失主包构建失败)。
