...
- 思考等级:**Shift+Tab 循环前进**快捷键(off→low→medium→high→off,从 off 开始;独立 Tab 不绑定)+ `/thinking` 命令(枚举选择器),状态栏"思维: <等级>"(off 隐藏);host-llm 会话级注入(请求未显式时),openai→reasoning_effort / anthropic→thinking budget_tokens(1024/4096/16384),off 均不发送(安全默认)。
