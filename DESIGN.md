...
| Web 设置面板「删除 provider」是死按钮 | ⏳ **待拍板**(2026-09-19 验收发现):后端 `DELETE /api/providers/{name}` 恒 **501**(M12 决策:单条删除不做、全清走 unset/clear),但面板仍渲染「删除」+ 二次确认并把 `upsert 覆盖即可(参考 M12 范围决策)` 抛给用户 → 修法二选一:**① 删掉按钮与 `deleteProvider` 调用面**;**② 改成「清空全部 provider」并补 REST**(= TUI 侧删 `provider.yaml` 的全清语义)。见 §14.1 第十九批记录 |
| 本机验收余项(TUI 42 / Web 10 / 文档 11 / 壳 8 / 其它 16) | ⏳ A 本机 109 条已跑 **21** 条(2026-09-19 第十九/二十批,见 §14.1),余 88 条;TUI 段需 pty 探针,壳段需 dmg |
