...
>
> **本批收口(2026-10-11)**:前置件 **E-A / E-C / E-D / E-E** 与实施方案 **DOC-1 / RST-1** 共 **6 项交付**(逐项交付行见上,全部带单测/e2e 与实测证据)。**暂存 1 项**(已登记前置,随时可开工):**SELF-1**(pdfium-WASM 自包含档;需有网实测体积/内存 + E-C 体积门决策,仅在「零外部依赖部署」需求成立时做)。**非代码侧**:X-2 三端真机验收(人工)、C2/C3 发行(等 repo 公开 + 首个 tag)。验证基线:全库 `go test ./... -race -count=1` 绿 + `go vet` 干净 + 前端 `vue-tsc` 0 错/单测 24 项 + `scripts/size-check.sh` 通过(SELF-1 路线 B 后本平台实测 43.32 MiB / gz 28.04 MiB,门 ≤46/≤30)。
>
...
> ⚠️ **R10 已知未闭环(诚实登记,非遗漏)**:① **shell 不做路径化**:workspace-write 档下 shell 命令仍可写任意路径(路径沙箱只管 `file_*` 参数;约束 shell 需命令 AST 级路径提取或内核级沙箱),当前靠危险模式 + 审批档兜底 —— 规划项:命令行解析提取路径 / macOS seatbelt / Linux bwrap。② 审批档↔沙箱联动仍为单向派生(strict→read-only、open→full-access,tool-files 已按 `EffectiveMode` 判定),绕过 `tools/pre-execute` 的工具直调不在覆盖内。③ 静态资源本身不鉴权(token 只保护 `/api/*`;入口页仅静态 shell,不含数据)。④ **工具能力声明缺位**:路径裁决按工具名表(`file_read/write/…`)分读写,新插件自定义名(如 `save_file`)不在表内即不受路径沙箱约束 —— 规划项:`sdk.Tool` 增可选能力接口(声明 path 参数名与读写意图)后改为能力驱动;⑤ 外部插件目录(`$GAH_HOME/plugins/`、`$GAH_HOME/ui-plugins/`)可写 = 用户替换插件即等同宿主代码执行(设计如此);`EXA_API_KEY` 默认不再下传外部 `web_search`,推荐改用 `$GAH_HOME/config/search.yaml`,或 `GAH_EXT_ENV_PASS=EXA_API_KEY` 显式放行(README 双语已注)。**⑥ 外部工具执行侧不可中断**(补跑切片后新增):RPC 只传参数不传 ctx(`serve.go` `t.Execute(context.Background(), …)`),长外部工具只能靠宿主侧超时兜底,回合取消不会传到外部进程 —— 规划项:协议级 cancel RPC 或把 ctx 截止时间随调用下发。**⑦ 同进程 TUI/Web 的 docview 资产表为进程内 map**(非 LRU):id 由 (文件, part) 哈希决定,重复预览不增长,但大量不同文档会常驻内存到进程退出 —— 规划项:随预览缓存一并淘汰。
