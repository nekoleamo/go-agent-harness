...
> **推进排期(第 1 组 = 本批,全部零新依赖)**:②-1 可见性收敛 + ②-4 入口委派契约测试 + ⑤-1 SPA CSP + ⑤-2 信任模型明示 + ①-A 环境 jail + ①-B 输出 flag 表。**第 2 组**:①-C(需批准依赖)、①-D、~~⑤-3~~ ✅ **已交付(2026-09-18 第十六批:产物 sha256 摘要 + 覆盖范围/降级原因如实下发)**、②-2 → **①-C 经 48 例探针实证否决**(控制流/复合命令/重定向已全部正确 DENY,AST 带来 ≈0 增量,故不引入依赖),改为**扩充写目标表 + `find -exec` 递归**;**第 3 组**:①-E 内核级沙箱 ✅ **已交付**(macOS seatbelt / Linux Landlock,见上方第 2/3 组交付块)。**仅记录不做**:②-3、⑤-4、①-F。**②-2 现状(2026-09-18 复核)**:仍未实施 —— `/sandbox sync` 运行期开关需要「插件写用户偏好」的通道,而 `internal/prefs` 只能由非插件代码 import(插件只 import `sdk` 是红线),须先补 `sdk` 侧偏好服务(登记为待评估,不硬塞)。验收点:②-1 状态/回显与 `EffectiveMode` 一致(web `/api/state` + TUI 状态栏 + `/sandbox`·`/approval` 回显);②-4 每个入口一条委派契约测试 + registry 必发 `pre-execute` 且 veto 阻止执行;⑤-1 CSP 头断言 + 前端回归(SSE/WS/预览);⑤-2 接口字段 + 文案;①-A jail 路径/权限/开关/失败路径用例 + 真实 `go build` 落 jail 的集成用例;①-B 每 flag 正反例 + git clone 单双操作数。
...
- ✅ **R10 ②-1 · 复核确认已交付(本轮无代码改动)**:审批档↔沙箱联动的**可见性收敛**在「R10 剩余规划项第 1 组」已落地并复核:后端 `/api/state` 下发 `sandbox_effective`/`sandbox_derived`(`web/server.go`),TUI 走 `sandboxDisplay`/`sandboxStatusText`/`approvalStatusText`,Web 由 `StatusBar.vue`(`sandboxLabel`)与 `InputBar.vue` 展示「实际生效档 + 派生标记」——状态展示与 `EffectiveMode()` 一致。**②-2(`/sandbox sync` 运行期开关)仍未实施**:需要 sdk 侧的偏好通道(插件→`internal/prefs` 会破「插件只 import sdk」红线),登记为待评估,见下方未实施清单。
- ⏳ **未实施/待评估(诚实登记,本轮未做)**:`NOND-W2b-β`(自定义数据根;须先证明「进 `$GAH_HOME`」的便携纪律不被绕过)与 `NOND-W5`(IM 投递,用户已决定暂缓)保持登记;`G-D6-1`(pdfium-on-WASM 光栅,破体积门)、`G-D6-3`(`excelize` xlsx,触发条件不可判定)维持**不做**决定;装机自动升级端到端与 `NOND-W1b`(PowerShell)仍**卡真机/Windows**。
