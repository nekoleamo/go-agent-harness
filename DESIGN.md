...
3. **发行自校验顺序缺陷(真缺陷,已修 `a357c0f`)**:`release-desktop.yml` 两个平台 job 的「上传前自校验」调 `verify-release.mjs --local dist-desktop --platform <p>`,而该步**硬要求 `dist-desktop/latest.json`** —— 总表要 merge job 才生成,平台 job 手上只有 `latest.<平台>.json` ⇒ **tag 一推这两个 job 必然红**(该步是 v0.1.4 之后才加的,今天首次真跑 tag 就撞上;v0.1.4 及以前没有这步)。修法:`--local` 且被 `--platform` 点名时**回退读平台级表**(两者 schema 相同,merge 只合并 `platforms`),并补 INFO 说明来源;无点名平台时仍是原来的清晰报错。本地三分支验证:① 只平台表 + `--platform` ⇒ 9 通过(含签名、包内 `gah.app 0.1.5`、sidecar 版本);② 无平台表无总表 ⇒ 退出码 1 + 指引;③ merge 后走总表 ⇒ 通过。4. **发版最终结果(2026-09-22)**:`v0.1.5` 已发并置为 **Latest**。Release 14 件 asset:桌面 `gah_0.1.5_aarch64.dmg`(25.6 MiB)/ `gah_0.1.5_x64-setup.exe`(23.3 MiB)/ `gah.app.tar.gz`(25.9 MiB)/ `latest.json`,CLI 六目标 tar.gz + zip,加 `checksums.txt`;updater 端点 `releases/latest/download/latest.json` 回 `version=0.1.5`、`platforms=[darwin-aarch64, windows-x86_64]`。`release-cli` 一次绿;`release-desktop` 平台 job 两个都绿、merge 经两次补发后绿。
5. **发行链上同源三处缝隙(第十八批加的两道自校验从未在真实 tag 上跑过,今天首次真跑)**:
   - ① **平台 job 上传前自校验硬要求总表** —— `verify-release.mjs --local` 只认 `dist-desktop/latest.json`,但总表要 merge job 才生成(平台 job 手上只有 `latest.<平台>.json`)⇒ tag 一推两平台必红。修:`--local` 且被 `--platform` 点名时**回退读平台级表**(两者 schema 相同,merge 只合并 `platforms`)。
   - ② **merge 回查未声明桌面矩阵** —— 加 `GAH_RELEASE_PLATFORMS: darwin-aarch64,windows-x86_64`(否则按 CLI 默认矩阵把「linux 不在桌面清单」判红)。
   - ③ **`--skip-artifacts` 仍按运行主机选平台** —— ubuntu runner 的主机平台是 `linux-x86_64`,拿它比桌面清单必然误报(本机 macOS 刚好在清单里,所以本地怎么跑都绿 —— 这是最难自己发现的一类)。修:只做结构/矩阵/指纹检查时**不做**主机平台选择判定,矩阵完整性仍由 env 感知的 `checkMatrix` 负责。
   - 另:merge job 补发(带 tag 输入)时改为 checkout **默认分支**的脚本(只有 tag push 才用 tag 自身)—— 否则修好的脚本会被 tag 上的旧副本搬回来,补发永远绿不了;且这与「不移动已消费的 tag」纪律一致。
   - **教训**:自校验步必须能在「最小上下文」(单平台产物、无总表、非目标主机)下跑通,否则等于给发布链加了一道必然失败的关卡;`--local`/`--skip-artifacts` 这类降级形态尤其要在本地按 CI 的真实主机语义各跑一遍。
6. **同版本覆盖提醒**:本机先前自建的 0.1.5(09:21 构建)与 CI 正式产物(16:36)同版本号 ⇒ updater **不会**提示升级;要用正式 CI 产物需手动覆盖安装,或等下次 0.1.6。(手动安装页 `gah_0.1.5_aarch64.dmg` / `gah_0.1.5_x64-setup.exe` 已带第五十六批溢出兜底 + 布局固定 + 步数上限可配。)

7. **待查(低优先,不阻塞发布)**:本地跑「线上全量产物校验」(不带 `--skip-artifacts`,需下载并解包 24.7 MiB 产物 + 验签)在本机 480s 未跑完(exit 124);此前一次跑完时曾出现 1 项 FAIL(疑似 `checksums.txt` 覆盖口径 —— 桌面 exe/dmg 不进 checksums 是**有意**的,签名才是其完整性来源,脚本以 INFO 说明)。待确认是「本机下载慢」还是真 FAIL。**发布判据不受影响**:CI 的两道门(`--local --platform` 平台产物校验 + `--skip-artifacts` 线上清单回查)本轮均已绿,平台 job 的包内版本/sidecar 版本也已逐项 OK。
...
1. **根因**:横向 gutter 归**容器**(视图只声明纵向内距 —— 变更/看板都是 `padding: 4px 0 12px`)。主视图容器 `.stream-slot` 自带 `12px 36px 20px`,所以切到全屏视图时留白正常;而停靠区外壳 `.dk-body` 完全**没有内距** ⇒ 同一组件在侧栏里就顶到面板边框。
2. **修**:`DockView` 增 `bodyPad`(缺省 `true`)⇒ `.dk-body { padding: 8px 12px 16px }`;「任务」是**列表型**面板(分隔线与悬停底色要通宽、内距自带 14px)⇒ App 传 `:body-pad="dock.panel !== 'jobs'"` 显式关掉,避免双重内距把分隔线挤成卡片。
3. **看板概览条改成卡片形(第二轮反馈:「看板标题加圆角、padding」)**:看板是**卡片型**视图,标题跟着卡片走 —— `.ov` 做成与卡片同形(`--r-card` 圆角 + `--line` 边框 + `10px 12px` 内距),与外层 `.grid` 的 10px 间距对齐。
   - **sticky + 圆角的坑**:滚动内容会从圆角缺口与 scrollport 内距带露出,而 sticky 被夹在**内容盒顶**(实测 `top: 0` 只能贴到停靠区内距之下,负边距拉不上去)⇒ 遮挡交给外层 `.ov-bar`:同色背景盖圆角缺口 + `box-shadow: 0 -12px 0 0` 向上补一条同色方块盖住内距带(阴影不参与布局 ⇒ 静止位置与原来完全一致)。
   - **变更/轨迹两条同款横条不动**:那两个视图的口径是「无卡片边框,发丝线 + 缩进」(工作台密度),看板才是卡片型,不一起改。
   - **验证(像素级,不靠目测)**:强制加高内容后滚到底读 PNG 像素 —— 条上方内距带(x1100/1200/1420,y68–74)全 `#ffffff`、左右上圆角缺口 `#ffffff`(无内容露头),而条下方立即是内容色(证明内容确实在滚、遮挡是真遮挡);静止态几何 = 卡片距停靠区左边框 **12px**、圆角 **10px**、内距 **10/12px**。
   - **第三轮反馈「背景里有一小块白色」**:病根是遮挡色硬写 `var(--bg)`(白)—— 主视图底色是白所以看不出来,停靠区底色是 `--bg2`(灰)⇒ 卡片四周的 gutter 与向上补的方块都是白块。新增语义 token **`--surface`**(=“卡片脚下的底色”,`style.css` 默认 `var(--bg)`,`DockView` 的 `.dk-body` 覆盖为 `var(--bg2)`),看板遮挡层与变更/轨迹两条同款横条的底色全改用它(变更/轨迹顺手补上同一套向上 12px 遮挡,它们也有“卷上去的内容从条上方露头”)。**卡片自身仍是白(`--bg`)** —— 白卡片叠在灰底上是设计意图,不该跟着变灰。
   - **验证(第二轮,像素级)**:停靠区里卡片 gutter、条上方内距带、左右上圆角缺口全部 `#f7f8fa`(与原面板底色完全一致)、卡片内 `#ffffff`;`--surface` 在 `.dk-body` 上实测解析为 `#f7f8fa`。主视图 `--surface` = `--bg` ⇒ 行为零变化。
4. **停靠区「任务」面板同步灰底(同轮追问「需要」)**:停靠模式下改用 `--surface`(列表型仍不带 gutter:分隔线/悬停要通宽),并把两处白底时代的配色一起重调:条目分隔线 `--line-faint` → **`--line`**(发丝线在灰底上几乎看不见),输出块 `--bg2` → **`--bg`**(它在白底时代是个浅灰块,到灰底上会与底色同化,改白底当“卡片面”)。浮动(抽屉)模式不变。
   - **验证(真实任务,非构造)**:让 agent 真起了 `sleep 90` 后台任务,在停靠区「任务」面板读值 —— 面板/条目底色 `#f7f8fa`(与另两个面板一致)、条目分隔线 `#e8eaee`、输出块 `#ffffff`(截图目视层级正常)。
4. **验证**(真机量取,不靠目测):重建 web 产物 + Playwright 开侧栏逐面板切 —— 变更/看板内容距面板左边框 **13px**(12px 内距 + 1px 边框),任务仍 **0**(通宽);截图目视留白协调、无裁切/色块错位。`npm test` 165 passed + `vue-tsc --noEmit` 干净。
5. **待办**:桌面端要重建 sidecar 才带上新产物(`web/dist` 内嵌进二进制)。

---

#### 交付记录(2026-09-22,第四十七批:压缩阈值改按模型窗口比例触发 + 长单轮截断 + 水位持久化)

**来源**:用户拍板口径 —— 触发 = 窗口填充 **80%**、上限按 **token 定 200K**、长单轮工具结果截断**要做**、水位持久化**一起做**。
...
5. **水位持久化**:`Load` / `RegisterCompressor` 把 `compressedUntil` 恢复到**最后一个** `session/summary` 的索引(此前恒 `-1`),加载后不再重复投影被摘要覆盖的原文。
6. **配置/样板**:新增 `trigger_ratio` / `max_tokens` / `chars_per_token`;**seed 25 → 26**(两份样板同步,guard 测试强制)。
...
3. **A-1b 体验缺口已补:`/export <路径>.html` 成功后默认自动打开** —— 此前只写文件、只回显路径(用户反馈“浏览器未自动打开”属实)。改:`host-internal-commands` 的 `cmdExport` 在 `.html` 分支写盘后调 `openPath`(平台 `open`/`xdg-open`/`rundll32`,只 Start 不 Wait);开关优先级 **env `GAH_EXPORT_OPEN`**(`0`/`false` 关) > manifest `data.export_open_browser` > 缺省 **true**;打不开只提示不报错 —— 文件已写好,SSH/无桌面场景不该把成功的导出变成失败。样板 `config/bundle-base.yaml` 与 seed 两份同步加 `data.export_open_browser: true`,**seed-version 24 → 25**。
   - 加固(同日):`openPath` 从“只 Start 不 Wait”改为 **同步执行并捕获 stderr** —— 打开失败此前是静默的(默认应用缺失/无图形环境/被策略拦长得一个样),现在回执带真原因;darwin 改用**绝对路径** `/usr/bin/open`,防桌面壳/sidecar 环境 PATH 被精简(`GAH_WEB_OPEN=0` 只关壳自己开页面,与导出无关)。
...
**根因一:占用 2233 的不是别人,是昨天的自己**。21 日验收用 nohup 起的 `./gah --profile web`(cwd `/private/tmp/acc`)被中断后**主进程死、4 个外部插件进程留成 PPID=1 的孤儿**;本机另积累 18 个更早的孤儿(单测/验收残留,最老 9/17),共 23 个。**真机对照实验**(同一 kill -9 手势):**旧二进制(2026-09-11 编译,go-plugin 时代)插件不退**;**当前源码(自研 stdio + net/rpc,插件读 stdin EOF 即 `os.Exit(0)`)三个插件 5 秒内全部自退** ⇒ 该缺陷在传输层去 go-plugin 时已被消除,残留来自未更新的**本地二进制**(日志里的 `plugin received interrupt signal, ignoring` 也是 go-plugin 特征,新传输层不再有)。这一条同时说明:**不要再把「孤儿进程」当成本代的开放缺陷**。

**根因二:`tool-mcp` 退出属配置入口,非缺陷**。`GAH_MCP_COMMANDS` 只在 `gah-data/env.sh`(便携纪律:启动 env 不落 shell profile),手工 `./gah` 不 source 它 ⇒ 插件看不到 server 清单即退出;走 `./start.sh` 正常。

**改动(2 文件,零新依赖)**:
- `start.sh`:**启动前清孤儿外部插件进程**(`PPID=1` 且 **argv[0] 精确匹配** `…/gah-data/plugins/<id>/<id>` —— 命令行里只是出现过该路径的无关进程不该被杀,首版按 command 子串匹配当场误杀了排障 shell,已收紧),`GAH_NO_REAP=1` 跳过;**只清插件,不清 `PPID=1` 的 gah 主进程** —— 那正是用户有意后台常驻的实例(`gah web` 常驻 + 浏览器访问是正当用法)。
- `web/server.go`:监听失败错误补出**可操作指引**(换端口 `GAH_WEB_ADDR` / 先清残留实例)。跨平台底层文案(linux `address already in use` / windows `Only one usage of each socket address…`)单独看等于没法下手 —— 而这个错误在本项目的历史成因九成是「上一个实例没死」。

**验证**:`go test ./web/ ./plugins/ui/ui-web-app/` **132 passed**(含新增断言「错误应给出换端口这条可操作指引」);`start.sh` 真机冒烟 —— 伪造孤儿(`exec -a` 精确路径、PPID=1)被识别并打印 PID,**诱饵进程**(命令行含该路径但 argv[0] 非插件)未被误杀;**本机 23 个历史孤儿全清、2233 释放**。

**遗留(操作项,非代码缺陷)**:仓库 `gah` 本地二进制停在 9/11 go-plugin 时代,跑它就会继续产生孤儿 ⇒ 重建即可(`go build -o gah ./cmd/gah`;`internal/embed.EnsurePlugins` 按 sha256 自动用新协议产物覆盖数据根里的旧插件,无需手工清理)。

#### 交付记录(2026-09-21,第四十三批:A-1a/A-1b 人眼项素材生成器 + 1 处测试窗口订正)

**动机**:A 本机剩 4 条纯人工中,**#42 P5 整体视觉**与 **#39 导出 HTML 观感**属"只能人看"的观感项 —— 自动化能做的只有"把要看的东西摆到桌上"(不假勾)。本批给出**可重放的素材生成器**,把两项的判据从"凭记忆找"变成"对着一份产物看"。

**新增**:`tests/tui_accept_human_eye_test.go`(默认 `Skip`,要 `GAH_EYE_DIR=<目录>` 才跑,不参与门禁)—— 用 `acctui`(llm-mock)跑一个含 **思考块 + 工具调用(40 行长输出)+ 收尾文本** 的回合,写出一套素材 + 一页式检查清单(`README.md`,内容与仓库同源):

| 产物 | 对应观感项 |
| --- | --- |
| `01-回合后-屏幕.txt` / `01-回合后-原始流.ansi.txt` | #42:一个完整回合的当前屏 + 带色原始流(看元素/顺序,颜色看真机) |
| `02-CtrlO-展开工具结果-屏幕.txt` | #42:`Ctrl+O` 展开长输出后的对齐/不粘连 |
| `03-CtrlT-思维块-屏幕.txt` | #42:思维块折叠/展开(素材思维段短,可能与 02 相同,已在 README 说明) |
| `04-会话导出.html` | #39:`/export <路径>.html` 的**真产物**(自包含单文件,实测 4580B:含思考段、工具调用、40 行长输出、中文无乱码、深底配色、无渐变/emoji) |
| `README.md` | 两项的「前置 → 做什么 → 看什么 → 判据 → 失败长什么样」+ 两条口径(80 列窄终端;屏幕文本来自不处理 `ESC[K` 的最小模型,只用于内容对照) |
...
| # | 人工项 | 前置 | 判据要点 |
| --- | --- | --- | --- |
| A-1a | #42 P5 整体视觉 | TUI + 任一 provider | 深灰底/▍竖线不断/折叠 ▲ 对齐/思考色区分/`Ctrl+O` 不串行/80 列不错位 |
...
- `TERM_PROGRAM=iTerm.app` → **OSC 9**;
- `TMUX=… + KITTY_WINDOW_ID=1` → **OSC 99 且被 DCS 包裹**(`ESC P tmux;`,不包裹会被 tmux 吞);
- `STY=… + TERM=screen-256color` → **bell 兜底**(screen 无 OSC 族匹配;裸 OSC 会被吞,兜底才是正确行为);
- 无任何信号(TERM=dumb)→ bell,且**不得**出现 OSC 9/777 落点。
新增 `tests/tui_accept_notify_targets_test.go`。

**② 顺手解决一个测试基建坑(值得复用)**:`probeEnv()` 把 `os.Environ()` 放在前面,而**进程读取 env 是"首个匹配生效"** → 宿主机终端的 `TERM=xterm-kitty`/`KITTY_WINDOW_ID` 会盖住用例设定(首版四例里两例因此误判为 OSC 99)。修法:新增 `envOver(base, over)`:把用例取值**放最前**、并从继承环境删同名键。凡与终端能力探测相关的 pty 用例都应走它。

**口径**:#88 自动化半边收口(选路矩阵 = 单测表 + 本端到端);真机"是否弹出来"仍属终端行为(人眼)。
