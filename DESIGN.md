...
| 项 | 行为 |
|---|---|
| home 目录 | 数据根解析链(2026-09 起):`GAH_HOME env` > 二进制同级 `gah-data/`(便携,首启自动新建并释放初始化内容);两者皆无(不可便携)= **启动显式报错退出**——~/.gah/TempDir 兑底已弃用(57739b1);运行态由 main Setenv GAH_HOME 贯通插件层 |
