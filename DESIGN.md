...
#### 暂缓项分析(2026-09-18:6 项暂缓 + 3 项待评估,逐一给出**可判定**解冻条件)
...
|---|---|---|---|---|
| `NOND-W5` | IM **单向**投递(通用 webhook / 飞书 / 企微 / 钉钉) | S | ⏸ 用户拍板暂缓 | **已基本具备,只差用户点头**(见 §1) |
...
**1. `NOND-W5` IM 单向投递(建议:解冻,量级 S)**
- **是什么**:把结果推到用户已打开的应用里。方案已定稿在 `docs/NONDEV_ROADMAP.md` §W5 —— 纯 Go `HTTP POST` JSON、**零新依赖**,四类目标(通用 webhook / 飞书自定义机器人 / 企微机器人 / 钉钉机器人);落位 `plugins/host/host-notify`(或 `tool-deliver`);配置入 `$GAH_HOME/config/notify.yaml`;挂点三处 = `/deliver <目标> <内容>` 命令、定时任务可选投递、工具 `notify({target,text})`;验收 = 三家真实 webhook 收到 + 凭据滤除(不继承 `*_API_KEY`,除非 `GAH_EXT_ENV_PASS`)+ 失败可读(HTTP 状态 + 平台错误码)。**不做双向对话网关**(锁死项,不重开)。
- **当初为何暂缓(2026-09-12)**:刚整体删除 IM 线(微信 iLink / QQ Bot v2 / 飞书自建应用三通道 + `im/` 运行时 + `sdk.IM*` + Web 端点 + 命令面,见上方 🗑️ 块),立刻加回同族能力显得反复;且当时定时任务的产出形态本身还没落地,W5 没有真实数据源。
- **现在的变化(解冻理由已成立)**:① `NOND-W4` 定时任务 + `NOND-W4b` 模型侧 `schedule` 工具 + `NOND-N1` 通知能力层**均已交付**(第十五/十六批)—— 触发源齐了,W5 不再需要新造任何触发点;W4 当时「投递随 W5 暂缓 → 产出只落会话记录」是暂缓期的**唯一形态**,现在多了 notice 流这一路;② N1 的 `sdk.Notice` + 环形缓冲 + 单调 `seq` + 桌面壳消费路径,本身就是「单向投递」的**进程内版本**,W5 = 同形状多接一个远端 sink,契约面可复用(不引第二套语义);③ 桌面壳通知已收敛为**单条**轮询器,多一个 sink 不再是「每个场景加一个轮询器」。
- **解冻条件**:用户拍板「要」(这是纯决策,不再是技术未知)。
- **风险**:① **凭据外发** —— 密钥入 `$GAH_HOME/config/`(0600)+ 投递子进程凭据滤除(验收点已含);② **出站网络** —— 单测用本地 HTTP 桩,e2e 不打真站点,三家真机 webhook 保留为人工过场;③ **静默丢最不可接受** —— 失败必须回显渠道 + 平台错误码并留痕。
...
**锁死项(明确不做,重启条件 = 无;改动它们等于改红线本身)**:双向 IM 对话网关 · Windows 内核级沙箱(无等价无特权机制)· 自研 cron 构造器 UI · 离线嵌入 WebView2(+≈120 MB)· 外部通知二进制(`terminal-notifier`/`BurntToast`/`notify-send`)· 终端内联图片(kitty 图形协议)· TUI scrollback 双形态 · 内嵌厂商 harness loop · A2A 1.0 · OpenClaw dashboard 整套 · pdfium TUI 位图 · OSC 9;4 进度通知。细目见 `S-P2-6`(S 组明确不做清单)与本节各行。
