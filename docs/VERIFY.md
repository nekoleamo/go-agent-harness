...
## P4-6 多行输入与外部编辑器(commit e30bb77)
...
## R8 数据根收紧(2026-09-16;仅 gah-data)
- [ ] 设置 `GAH_HOME=<任意路径>` 启动 → gah 仍用**二进制同级 gah-data/**(env 被忽略,不一致时日志告警 `忽略 GAH_HOME env`);数据实际读写均在 gah-data/
- [ ] 无 gah-data 的空目录放 gah → 首启自动创建 gah-data/ 并释放 config 样板+插件;数据随目录整体迁移(升级只换 gah 单文件)
- [ ] gah 二进制所在目录只读(不可建 gah-data)→ 启动显式报错退出(提示"数据根仅支持 gah-data"),不落 ~/.gah/任何系统根
- [ ] 桌面壳(gah .app):数据根 = Contents/MacOS/gah-data(随壳);升级 .app 前先 `/backup` 保留数据
