...
| 26 ✅ | **M16.9 便携数据根与纪律**(2026-09) | ⚠️ main | S–M | ✅ 已交付:homeDir 便携自动发现(gah 同级 gah-data,GAH_HOME>便携>~/.gah>TempDir)+ 迁移(旧 ~/.gah→gah-data,env 入 gah-data/env.sh,start.sh 启动)+ 便携纪律入 AGENTS/PLUGIN_DEV 规范。细节见 DESIGN §14.1 M16.9。**后续变更(57739b1)**:~/.gah/TempDir 兑底已弃用,不可便携时 boot 显式报错退出 | ✅ 升级只替换 gah 单文件;数据/密钥/env 全随 gah-data;-race 全绿 |
...
- 2026-09-16:R7 桌面零成本发行工程(C2/C3 变体;DESIGN §14.1 R7)+ **R8 数据根收紧**(DESIGN §14.1 R8):排除 GAH_HOME env,数据根唯一 = 二进制同级 gah-data(无则自动创建,禁 ~/.gah);cmd/gah homeDir + 桌面壳(appDataHome 删除)+ pty 探针同步;文档/README 中英同步;全库 44 包 -race 绿 + cargo check 通过。
