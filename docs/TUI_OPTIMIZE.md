...
### 1.8 选择器内过滤(参数级直接打字即搜)✅ 已实施
- 问题:参数级选择(会话/插件/工作区等选项列表)选项多时只能滚动,项多仍低效;此时打字会写进
  命令文本并退出选择器。
- 方案:参数级选择(Level>0)激活时直接打字 = **子串即时过滤**(Pick.Filter,匹配 Value/Desc,
  大小写不敏感,从 All 全量重算 Items);过滤状态行占提示区首行(“过滤: q → k/N 项”,窗口相应
  减 1 行);退格删过滤词、清空恢复全量;Esc 先清过滤(再按退出选择器);过滤无匹配时回车不误提交;
  过滤集仍走 §1.7 滚动窗口。命令名一级(Level 0)保持既有输入即前缀过滤。
- 已落地:tui/picker.go(Filter/All 字段 + filterOptions)+ tui/model.go(pickFilterType/
