...
> 状态:✅ S1.1/S1.2/S1.3/S1.5/S1.6 已实施;⏳ 剩余 S1.4(S2/S3 见下,按 S1→S3 逐项独立验收)。
...
2. → S1.4(Markdown 轻渲染):输入/文本态机已单测先行(S1.3),可接力。
3. → S2(分组/组件化):组件化完成前避免大面积改 render。
4. → S3(形态):S2.2 完成后评估,作为独立里程碑。
