...
| 命令行版(任意平台) | `gah_<版本>_<系统>_<架构>.tar.gz` | 解压即用的单文件二进制 |
...
> **数据在哪 / 升级前必读**:数据(会话、记忆、计划、密钥)全在**应用目录内的 `gah-data/`**。
> 桌面版升级是**整包替换**,所以升级前请先备份到应用目录之外:
> 在对话里执行 `/backup ~/gah-备份`(路径必须在应用目录之外),升级后用 `/backup restore <名字>` 恢复;
> 桌面壳检查更新时也会**自动先备份**到 `~/gah-upgrade-backup/`,备份失败会**取消升级**(不拿数据冒险)。
> 卸载则连安装目录一起删除(数据不残留)。命令行版不受此影响:升级只替换 `gah` 单文件,`gah-data/` 原地保留。
